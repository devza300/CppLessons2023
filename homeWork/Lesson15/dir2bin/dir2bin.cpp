...
QList<Entry> dir2bin::toBin(const QString &dir)
{
    QDir root(dir);
    return exploreDir(root, dir);
}

void dir2bin::toBin(const QString &dir, const QString &bin)
{
    auto entries = toBin(dir);
...
void dir2bin::toBin(const QList<Entry> &entries, const QString &bin)
{
    DataList<Entry> dataList(entries);
...
void dir2bin::toDir(const QList<Entry> &entries, const QString &dir)
{
    QDir root(dir);
    if (!root.exists())
